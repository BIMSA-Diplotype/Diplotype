...
network_conversion <- function(result){
  ...
  effect = colSums(result$predict[,3:n]*(result$predict$x[2]-result$predict$x[1]))
 ...
 # temp= temp[-which(effect[-c(1:2)] < effect[1]*0.1),]
...
network_plot <- function(result, title = NULL, maxeffect = NULL, type = NULL){
  ...
  #layout
  l <- layout.circle(net)
  pdf(paste0(substr(V(net)$name[1],4,100),"_net.pdf"), width = 8, height = 8)
  
 plot.igraph(net,
              #vertex.label= V(net)$name,
             vertex.label=c("Flesh","Skin","Leaf"),
             vertex.label=c(" "," "," "),
             edge.arrow.size=2.5,
  ...
  title(main=  substr(V(net)$name[1],4,100), cex.main = 3,font.main = 1)
  dev.off()
...
net_all = lapply(rdiploid_chra[[10418]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 

 net_all = lapply(rdiploid_chra[[5]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 
 net_all = lapply(rdiploid_chra[[316]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 
 ...
 net_all = lapply(rdiploid_chrb[[10418]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 
 
 net_all = lapply(rdiploid_chrb[[5]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 
 net_all = lapply(rdiploid_chrb[[316]][c(1,2,3)], network_conversion) 
 network_plot(net_all) 
